function result = DBsim_eps(alpha,con_matrix, N,iter, p_ini, b,eps,beta,pmal)
...
%    index_h = 1:num_h;   
    ...
    b_user = randperm(num_b, round(num_b * pmal));
    b_user =  num_h + b_user;
    action_table(b_user) = 1;  %pmal可以修改
...
%     end
    ...
    count = 1;
    result(count) = sum(action_table(1:num_h))/num_h; %%%结果计算修改pl
    count = count + 1;
...
%
        ...
        result(count) = sum(action_table(1:num_h))/num_h; %%%%%修改
        count = count + 1;
        ...
        action_table(index_b) = 0;
        b_user = randperm(num_b, round(num_b * pmal));
        b_user =  num_h + b_user;
        action_table(b_user) = 1;  %%%%%恶意用户更新策略
